    ...
    , tick_{ 0 }
{
    ...
    server_ = std::make_unique<server::Server>(this);
    client_ = std::make_unique<client::Client>(this);
}
...
{
    static std::mutex extension_mutex;
    std::lock_guard<std::mutex> lock(extension_mutex);
    
    spdlog::trace("Checking if extension with UID 0x{:x} should be ignored", ext->get_uid());
...
void Core::run()
{
    ...
    while (run_) {
        ...
        event_dispatcher_.dispatch(EventTick{});
        for (const auto& ext : std::views::values(extensions_)) {
            ext->tick();
        }
