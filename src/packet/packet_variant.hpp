...
public:
        ...
        for (uint8_t i{ 0 }; i < size; i++) {
            ...
            try {
                if (type == static_cast<uint8_t>(VariantType::FLOAT)) {
                    ...
                    glm::vec2 value{};
                    if (!byte_stream.read(value.x) || !byte_stream.read(value.y)) {
                        return false;
                    }
                    variants_.emplace_back(value);
                }
                    ...
                    glm::vec3 value{};
                    if (!byte_stream.read(value.x) || !byte_stream.read(value.y) || !byte_stream.read(value.z)) {
                        return false;
                    }
                    variants_.emplace_back(value);
                }
    ...
    [[nodiscard]] bool empty() const { return variants_.empty(); }
